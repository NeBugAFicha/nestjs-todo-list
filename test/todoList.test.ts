...
describe('todoList test', () => {
  ...
  it(`Created 2 task lists with 3 tasks in each and give Read, Create Grants to EgorDeveroper, Update, Delete Grants to AnotherUser,
    ...
    const { body: { data: { token } } } = await request(app.getHttpServer()).post('/registration').send({ login: 'AnotherUser2', password: '12341234' }).expect(201);
    // console.log((await request(app.getHttpServer()).post('/login').send({ login: 'AnotherUser2', password: '12341234' }).expect(201)).body);

    //Create 2 new task lists for new user AnotherUser2
    const responses = await Promise.all([
      request(app.getHttpServer()).post('/task_list').send({ name: 'AnotherUser2_task_list' }).set('token', token).expect(201),
      request(app.getHttpServer()).post('/task_list').send({ name: 'AnotherUser2_task_list2' }).set('token', token).expect(201),
    ]);
    ...
    const [
      ...
      request(app.getHttpServer()).post('/login').send({ login: 'EgorDeveloper', password: '12341234' }).expect(201),
      request(app.getHttpServer()).post('/login').send({ login: 'AnotherUser', password: '12341234' }).expect(201),
    ]);
    ...
    const [
      ...
      request(app.getHttpServer()).get(`/task_list/${responses[0].body.data.id}`).set('token', token2).expect(200),
      request(app.getHttpServer()).get(`/task_list/${responses[1].body.data.id}`).set('token', token2).expect(200),
    ]);
    ...
    expect(createdError).toBe('No Owner or Create Grant or task list doesnt exist');
    expect(deletedError).toBe('No Owner or Delete Grant or task list doesnt exist');
